...
useEffect(()=>{fit()},[])
const focus=(id)=>{const q=by[id],e=ref.current;if(!q||!e)return;const nz=clamp(Math.max(z,window.innerWidth<760?.62:.58),MIN,MAX);setZ(nz);setPan({x:e.clientWidth/2-(q.x+q.w/2)*nz,y:e.clientHeight/2-(q.y+q.h/2)*nz});setSel(id)}
...
return <main className="ki-page"><header className="ki-header"><button type="button" className="ki-back" onClick={()=>{window.location.hash='#/semestre/5/ontologia-ii'}}>← ONTOLOGÍA II</button><div className="ki-header-copy"><span>IMMANUEL KANT · CRÍTICA DE LA RAZÓN PURA</span><h1>Introducción</h1><p>Mapa didáctico 2D · experiencia, a priori, juicios y problema trascendental</p></div><div className="ki-date"><span>LECTURA</span><strong>09 · SEP · 2026</strong></div></header><section className="ki-study-strip"><div><span>01</span><p><strong>Arrastre</strong> el lienzo para recorrer el argumento.</p></div><div><span>02</span><p><strong>Seleccione</strong> un nodo para abrir su explicación.</p></div><div><span>03</span><p><strong>Siga la ruta</strong> para reconstruir el razonamiento.</p></div></section><section className={`ki-workspace ${n?'has-inspector':''}`}><div className="ki-viewport" ref={ref} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up}><div className="ki-toolbar"><button type="button" onClick={()=>zoom(.12)}>+</button><button type="button" onClick={()=>zoom(-.12)}>−</button><button type="button" onClick={fit}>AJUSTAR</button><button type="button" className={route?'active':''} onClick={()=>setRoute(v=>!v)}>RUTA</button></div><div className="ki-zoom-readout">{Math.round(z*100)}%</div><div className="ki-world" style={{width:W,height:H,transform:`translate(${pan.x}px,${pan.y}px) scale(${z})`}}>{regions.map(r=><div key={r.id} className="ki-region" style={{left:r.x,top:r.y,width:r.w,height:r.h,'--ki-region':r.c}}><span>{r.label}</span></div>)}<svg className="ki-edges" viewBox={`0 0 ${W} ${H}`}><defs><marker id="ki-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z"/></marker></defs>{edges.map(([a,b,l])=>{const A=by[a],B=by[b];return <g key={`${a}-${b}`} className={route&&set.has(a)&&set.has(b)?'path-edge':''}><path d={path(A,B)} markerEnd="url(#ki-arrow)"/><text x={(center(A).x+center(B).x)/2} y={(center(A).y+center(B).y)/2-10}>{l}</text></g>})}</svg>{nodes.map(q=><button key={q.id} type="button" className={`ki-node ${q.hero?'hero':''} ${q.id===sel?'active':''} ${route&&set.has(q.id)?'in-path':''}`} style={{left:q.x,top:q.y,width:q.w,minHeight:q.h,'--ki-node':colors[q.tone]}} onClick={e=>{e.stopPropagation();setSel(q.id)}} onDoubleClick={()=>focus(q.id)}><span className="ki-node-eyebrow">{q.e}</span><strong>{q.t}</strong><em>{q.s}</em>{q.hero&&<span className="ki-node-star">★</span>}</button>)}</div></div><Inspector n={n} i={i} close={()=>setSel(null)} prev={()=>step(-1)} next={()=>step(1)}/></section><footer className="ki-footer"><span>CADENA DE MEMORIA</span><p>Experiencia → a priori → necesidad + universalidad → analítico / sintético → sintético a priori → ¿cómo es posible? → matemática / física / metafísica → crítica → trascendental → sensibilidad + entendimiento.</p></footer></main>}
